...
async def run_web_server():
    ...
    await runner.setup()
    # Host is set to 0.0.0.0 to be accessible within the container
    site = web.TCPSite(runner, '0.0.0.0', PORT)
    logger.info(f"Starting web server on port {PORT}...")
    await site.start()
    # Keep the server running indefinitely
    while True:
        await asyncio.sleep(3600) # Sleep for 1 hour, the task will keep running
