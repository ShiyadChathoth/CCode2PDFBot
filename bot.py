    ...
    try:
        ...
        <head>
            ...
            <style>
                ...
                .terminal-line {{
                    ...
                    color: #00ff00;      /* Bright green text for all terminal content */
                }}
                .terminal-prompt {{
                    color: #00ff00;      /* Bright green text for prompt */
                    font-weight: normal;
                }}
        ...
        while i < len(terminal_log):
            ...
            if entry_type == 'prompt':
                # Special handling for command prompts
                terminal_content += f'<span class="terminal-prompt">{html.escape(content)}</span>'
            else:
                ...
                if "PID" in content and "Burst Time" in content and "Turnaround Time" in content:
                    ...
                    table_content = '<div class="terminal-table">'
                    
                    # Process header
                    headers = content.strip().split()
                    table_content += '<div class="terminal-row">'
                    for header in headers:
                        table_content += f'<div class="terminal-cell terminal-header">{html.escape(header)}</div>'
                    table_content += '</div>'
                    ...
                    while j < len(terminal_log) and terminal_log[j]['type'] == 'output':
                        ...
                        if re.match(r'^\s*\d+\s+\d+', row_content):
                            ...
                            table_content += '<div class="terminal-row">'
                            for cell in cells:
                                table_content += f'<div class="terminal-cell">{html.escape(cell)}</div>'
                            table_content += '</div>'
                            j += 1
                        else:
                            break
